...
function identity(s){
 if(s.view==='finish')return `<header class="utility-header finish-header"><div><h3>Finish farrowing</h3><p>${s.tag} · ${s.pen} · Parity ${s.parity}</p></div>${SentriUI.iconButton({action:'dismiss',icon:roomIcon('close'),label:'Close finish drawer'})}</header>`;
 ...
 return `<header class="utility-header sow-header"><div><h3>${btn('profile',esc(s.tag)+icon('chevron'),'sow-title-link').replace('<button',`<button aria-label="View sow ${s.tag} details"`)}</h3><p class="sow-context"><span class="sow-location">${esc(s.pen)} / Parity ${s.parity}</span><span>${esc(recordStamp(s)||state)}</span></p></div><div class="sow-header-actions">${SentriUI.iconButton({action:'dismiss',icon:roomIcon('close'),label:'Close sow drawer'})}</div></header>`;
}
...
function penSubpage(s,c){
 ...
 if(s.view==='roomPenFault')return sheet('Report fault',`${SentriUI.pickerField({label:'Device',value:draft.device||'',placeholder:'Choose device',key:'pen:device',className:'pen-field'})}${draft.device==='Other'?SentriUI.field({label:'Device name',control:`<input data-pen-field="name" value="${esc(draft.name||'')}">`,className:'pen-field'}):''}${SentriUI.field({label:'Description',control:`<textarea data-pen-field="description" placeholder="What is broken or not working?">${esc(draft.description||'')}</textarea>`,className:'pen-field'})}`,btn('room-pen-save-fault','Report fault','button primary','',!validPenFault(draft)));
 if(s.view==='roomPenFaultRecord'){
  ...
  return sheet(f.device,`<p class="pen-detail-copy">${esc(f.description)}</p><p class="pen-detail-meta">${f.open?'Open':'Resolved'} · Reported ${f.time} · ${esc(f.who)}</p>${f.open?SentriUI.field({label:'Resolution note · optional',control:'<textarea data-pen-field="resolution"></textarea>',className:'pen-field'}):`<p class="pen-detail-copy">${esc(f.resolution||'')}</p>`}`,f.open?btn('room-pen-resolve-fault','Resolve fault','button primary'):'');
 }
 return null;
}
...
function pigletCarePage(s){
 ...
 const tabs=SentriUI.segment({options:[['care','Care'],['records','Piglet records']],active:s.pigletTab,action:'pigletTab',className:'feature-tabs',ariaLabel:'Piglet processing sections'});
