...
function healthStateFields(f){return fieldSelect('Care','triage',careOptions,f.triage||'None');}
...
function main(c){const n=c.selected.size+c.selectedPens.size,filtered=pigs(c).filter(p=>matches(c,p)),filterN=(c.batchFilter!=='All')+(c.stage!=='All')+(c.healthFilter!=='All')+c.manualOnly+(c.triageFilter!=='All')+(c.healthStatus!=='All');return `<div class="inspect-main"${c.view!=='list'?' inert':''}>${status()}<header class="inspect-header task-header">${ib('home','chevron','Back to Today').replace('icon-button','icon-button task-return')}<div class="inspection-page-title"><h2>Inspection</h2><small>${unitName(c)}</small></div>${button('finish',c.checkedIn?'Check in again':'Finish walk','text-button finish-walk')}</header><div class="inspect-scroll" role="region" aria-label="${unitName(c)} inspection list" tabindex="-1"><div class="inspect-intro task-activity"><p>Last check-in <strong>${c.checkedIn?'09:41':'06:40'}</strong><span> · G. Hansen</span></p>${button('history',icon('clock')+'<span>Log</span>','task-history').replace('<button','<button aria-label="Updates this walk"')}</div><div class="inspect-controls"><div class="room-tabs">${['All','Health','Feed'].map(v=>button('lens',`<span>${v}</span><span class="lens-count">${filtered.filter(p=>matchesLens(c,p,v)).length}</span>`,'',v).replace('<button',`<button aria-pressed="${c.lens===v}"`)).join('')}</div>${button('filters',icon('filter')+(filterN?`<span class="filter-badge">${filterN}</span>`:''),'room-filter-button').replace('<button','<button aria-label="Filter pigs"')}</div>${filterN?`<div class="inspection-filter-summary"><span>${[c.batchFilter!=='All'?'Batch '+c.batchFilter:'',c.stage!=='All'?c.stage:'',c.healthFilter!=='All'?c.healthFilter:'',c.healthStatus!=='All'?(c.healthStatus==='ongoing'?'No action needed':'Needs attention'):'',c.manualOnly?'Manual feed':'',c.triageFilter!=='All'?careLabel(c.triageFilter):''].filter(Boolean).join(' · ')}</span>${button('reset-filters','Clear','text-button')}</div>`:''}<div class="inspect-list">${c.pens.map(pe=>{const matchesInPen=visiblePigs(c,pe),rows=displayPigs(c,pe);if(!matchesInPen.length&&!(c.reviewOnly&&c.selectedPens.has(pe.id)))return '';const num=rows.filter(p=>c.selected.has(p.id)).length;return `<section class="inspect-pen" data-pen="${pe.id}"><header class="inspect-pen-head">${penEntry(pe)}${penFeedEntry(pe)}<label class="check-target pen-select-target"><input type="checkbox" data-pen-select="${pe.id}" data-mixed="${num>0&&num<rows.length}" aria-label="Select pen ${pe.id} and ${rows.length} displayed ${rows.length===1?'pig':'pigs'}"${c.selectedPens.has(pe.id)?' checked':''}></label></header>${pe.issue?`<div class="pen-attention">${icon('alert')}${esc(pe.issue)} · ${pe.issueTime}</div>`:''}${rows.map(p=>pigRow(c,p)).join('')}</section>`;}).join('')||`<div class="empty-inspect"><strong>No pigs match</strong>${button('reset-filters','Show all pigs','text-button')}</div>`}</div></div><nav class="inspect-dock room-dock" aria-label="Inspection tools">${n?`${ib('clear','close','Clear selection')}<div class="dock-selection-summary"><p>Selected · <strong>${selectionLabel(c)}</strong></p>${button('review-sheet','View selected','selection-review')}</div>${button('actions','Actions','button primary selection-action')}`:`${button('grid',icon('grid')+'<span>'+(c.currentPen||'Pens')+'<small>Go to pen</small></span>','dock-pen').replace('<button','<button aria-label="Go to pen"')}${button('scan',icon('scan')+'<span>Scan ear tag</span>','button primary dock-scan')}${button('search',icon('search'),'dock-search').replace('<button','<button aria-label="Search ear tag"')}`}</nav></div>`;}
function fieldSelect(label,key,items,value){return `<label class="field">${label}<select data-v2="${key}">${items.map(([v,t])=>`<option value="${esc(v)}"${v===value?' selected':''}>${t}</option>`).join('')}</select></label>`;}
...
function healthForm(c){const f=c.form;return sheet(c,'Record health',scope(c),`<div class="health-tabs">${['Disease','Symptom'].map(k=>button('health-kind',`${k==='Disease'?'Diseases':'Symptoms'} <span>${f.conditions.filter(n=>conditionDefinition(f,n)?.kind===k).length}</span>`,'',k).replace('<button',`<button aria-pressed="${f.kind===k}"`)).join('')}</div><label class="field catalog-search">Search ${f.kind.toLowerCase()}s<input type="search" data-v2="search" value="${esc(f.search||'')}" placeholder="Name, category or abbreviation"></label><div class="health-catalog">${healthList(c)}</div><p class="selected-conditions">${f.conditions.length?f.conditions.map(esc).join(' · '):'Choose diseases or symptoms'}</p>${healthStateFields(f)}<p class="quiet-note">Existing cases keep their original start date.</p>`,footer(back(),button('save-health',`Record · ${countLabel(f.subjects.length)}`,'button primary','',!f.conditions.length)));}
...
function bodyForm(c){
 ...
 fieldSelect(single?'Adjustment':'Observed condition','band',[['',single?'Choose adjustment':'Choose condition'],...(single?[['custom','Other reason · custom amount']]:[]),...bands,...(single?[['standard','Clear adjustment · return to base plan']]:[])],f.band)+
 ...
 '<div class="detail-line plan-start"><span>Starts</span><span>Today · 12 Sep</span></div>'+fieldSelect('Ends','end',[['','Choose an end'],['2026-09-15','15 Sep · after 3 days'],['2026-09-19','19 Sep · after 7 days'],['2026-09-26','26 Sep · after 14 days'],...(!custom?[['when recovered','When the condition is resolved']]:[])],f.end)+
...
function pigFeedFacts(p,pe){
 if(pe.mode==='unknown')return '<section class="detail-section reading-section pig-feeding-facts"><div class="detail-heading"><h4 class="section-title">'+icon('feed')+'Feeding</h4></div><p class="detail-empty facts-surface">No feeding plan recorded.</p></section>';
 ...
 return '<section class="detail-section reading-section pig-feeding-facts"><div class="detail-heading"><h4 class="section-title">'+icon('feed')+'Feeding</h4>'+button('pig-feed','View plan '+icon('chevron'),'profile-details-link',p.id)+'</div><div class="facts-surface"><div class="pig-feed-fact-row"><span class="pig-feed-copy"><strong>'+esc(pe.formula)+'</strong><small>'+esc(context)+'</small></span><span class="pig-feed-amount">'+amount+'</span></div>'+(tags.length?'<div class="feeding-status-tags">'+tags.map(recordedTag).join('')+'</div>':'')+'</div></section>';
}
...
function bulkSelect(c,label,key,options){return '<label class="field">'+label+'<select data-bulk-key="'+key+'">'+options.map(([v,t])=>'<option value="'+esc(v)+'"'+(c.form[key]===v?' selected':'')+'>'+esc(t)+'</option>').join('')+'</select></label>';}
...
function recordOptionalEditor(c){
 ...
 const field=key==='target'?'<label class="field">Condition<select data-record-optional-input><option value="">No linked condition</option>'+conditions.map(n=>'<option'+(value===n?' selected':'')+'>'+esc(n)+'</option>').join('')+'</select></label>':key==='note'?'<label class="field">Note<textarea rows="3" data-record-optional-input placeholder="Write a note">'+esc(value)+'</textarea></label>':'<label class="field">Brand<input data-record-optional-input value="'+esc(value)+'" placeholder="Brand or formulation"></label>';
...
function productionDispositionPage(c){
 ...
 return sheet(c,title,picked(c)[0].id+' · '+penOf(c,picked(c)[0].id).id,'<div class="sow-action-warning">'+icon('alert')+'<p>'+copy+'</p></div>'+fieldSelect('Reason','dispositionReason',options,f.dispositionReason||'')+'<label class="field">Note · optional<textarea data-v2="dispositionNote">'+esc(f.dispositionNote||'')+'</textarea></label>',footer(back(),button(removing?'save-batch-removal':'save-not-in-pig',removing?'Remove from batch':'Save outcome','button primary','',!f.dispositionReason)));
}
function batchMembershipPage(c){
 ...
 const body='<section class="batch-membership-card facts-surface"><span>Current batch</span><strong>'+(current?'Batch '+esc(current.id):'No batch')+'</strong>'+(current?'<small>'+esc(current.stage)+' · '+current.pigIds.length+' pigs</small>':'<small>This pig is not assigned to a production batch.</small>')+'</section>'+(choices.length?fieldSelect(current?'Change membership':'Add to batch','batchTarget',[['','Choose a batch'],...choices],f.batchTarget||''):'<p class="detail-empty">No production batches are available.</p>')+'<label class="field">Note · optional<textarea data-v2="batchNote" placeholder="Why is this changing?">'+esc(f.batchNote||'')+'</textarea></label>';
...
function transferPage(c){const p=picked(c)[0],d=c.transferDraft||{},units=c.transferDestinations||[],pens=units.find(u=>u.id===d.unit)?.pens||[];return sheet(c,'Transfer sow',p.id+' · '+penOf(c,p.id).id,`<div class="transfer-origin"><span>From</span><strong>${esc(c.transferOrigin?.name||'')} · ${esc(penOf(c,p.id).id)}</strong></div><label class="field">Destination unit<select data-transfer="unit"><option value="">Choose unit</option>${units.map(u=>`<option value="${esc(u.id)}"${d.unit===u.id?' selected':''}>${esc(u.name)}</option>`).join('')}</select></label><label class="field">Destination pen<select data-transfer="pen"${!d.unit?' disabled':''}><option value="">Choose pen</option>${pens.filter(pen=>!(d.unit===c.transferOrigin?.unit&&pen===penOf(c,p.id).id)).map(pen=>`<option value="${esc(pen)}"${d.pen===pen?' selected':''}>${esc(pen)}</option>`).join('')}</select></label>`,footer(back(),button('save-transfer','Transfer sow','button primary','',!transferReady(c))));}
...
function overlay(c){if(c.logDatePicker)return '<div class="drawer-background" inert aria-hidden="true">'+overlay({...c,logDatePicker:false})+'</div>'+logDatePicker(c);if(['record-optional','feed-date','feed-note','medicine-picker','bulk-health-picker'].includes(c.view)&&!c.isDrawerBackground){const parent={...c,view:c.view==='record-optional'?c.form.optionalReturn:c.view==='medicine-picker'?'treatment':c.view==='bulk-health-picker'?'health':'feed-editor',isDrawerBackground:true};return '<div class="drawer-background" inert aria-hidden="true">'+overlay(parent)+'</div>'+overlay({...c,isDrawerBackground:true});}if(c.view==='medicine-picker')return medicinePicker(c);if(c.view==='record-optional')return recordOptionalEditor(c);if(c.form?.recordEditor&&bulkActionKinds.includes(c.view))return bulkActionPage(c);if(c.view==='bulk-health-picker')return bulkHealthPicker(c);if(c.view==='sow-transfer')return transferPage(c);if(c.view==='unavailable-actions')return unavailableActionsPage(c);if(c.view==='measurements-menu')return measurementsMenu(c);if(c.view==='miscarriage')return miscarriagePage(c);if(c.view==='production-task')return productionTaskPage(c);if(c.view==='batch-membership')return batchMembershipPage(c);if(['batch-removal','not-in-pig'].includes(c.view))return productionDispositionPage(c);if(c.view==='feed-date'||c.view==='feed-note')return feedOptionalEditor(c);if(c.view==='feed-breakdown')return feedBreakdown(c);if(c.view==='pig-production-batch')return productionBatchPage(c);if(c.view==='feed-editor')return feedEditor(c);const recordView=recordFlowOverlay(c);if(recordView!==null)return recordView;const revised=round3Overlay(c);if(revised!==null)return revised;const f=c.form;
 if(c.view==='actions')return actionsPage(c);
 if(c.view==='treatment')return sheet(c,f.taskId==='vaccination'?'Record vaccination':'Record treatment',f.returnFinding?c.pigId+' · '+f.returnFinding:scope(c),`<label class="field">${f.taskId==='vaccination'?'Vaccine':'Medicine'}<input data-v2="medicine" value="${esc(f.medicine||'')}" placeholder="${f.taskId==='vaccination'?'Vaccine administered':'Medicine administered'}"></label><label class="field">Brand / formulation · optional<input data-v2="brand" value="${esc(f.brand||'')}"></label>${fieldSelect('Method','method',[['','Choose method'],['Injection','Injection'],['Oral','Oral'],['Topical','Topical'],['In feed','In feed'],['In water','In water']],f.method||'')}<div class="plan-meta"><label class="field">Dose per pig<input type="number" min="0" step="any" data-v2="dose" value="${esc(f.dose||'')}"></label>${fieldSelect('Unit','doseUnit',[['','Choose unit'],['mL','mL'],['mg','mg'],['g','g']],f.doseUnit||'')}</div>${f.taskId==='vaccination'?'':f.returnFinding?'<div class="detail-line"><span>For this finding</span><strong>'+esc(f.returnFinding)+'</strong></div>':fieldSelect('Target condition · optional','target',[['','No linked condition'],...[...new Set(f.subjects.flatMap(id=>cases(pig(c,id)).map(k=>k.name)))].map(n=>[n,n])],f.target||'')}<p class="quiet-note">${f.taskId==='vaccination'?'Record the vaccination administered for this task.':'Records the treatment already administered to these pigs. A target links only to pigs carrying that condition.'}</p>`,footer(back(),button('save-treatment',f.taskId?'Save and complete task':`Record · ${countLabel(f.subjects.length)}`,'button primary','',!valid(c))));
 if(c.view==='health')return healthForm(c);
 if(c.view==='body'||c.view==='individual-plan')return bodyForm(c);
 if(c.view==='filters'){const d=c.filterDraft;return sheet(c,'Filter pigs',unitName(c),`${fieldSelect('Production stage','stage',[['All','All production stages'],...stageList(c).map(s=>[s,s])],d.stage)}${fieldSelect('Disease or symptom','healthFilter',[['All','Any condition'],...[...new Set(pigs(c).flatMap(p=>cases(p).map(k=>k.name)))].map(k=>[k,k])],d.healthFilter)}${fieldSelect('Care','triageFilter',[['All','Any care'],...careOptions],d.triageFilter)}`,footer(button('filter-reset','Reset','button secondary'),button('filter-apply','Show pigs','button primary')));}
 ...
 if(c.view==='resolve'){const rows=f.subjects.flatMap(id=>cases(pig(c,id)).map(k=>({id,k})));return sheet(c,'Resolve conditions',scope(c),rows.map(({id,k})=>`<div class="resolve-case"><div><strong>${id}</strong><span>${esc(k.name)} · day ${k.day}</span>${k.feedLinked?'<small>Recovery ends this case’s feed adjustment</small>':''}</div><select data-verdict="${id}|${k.name}" aria-label="Outcome for ${k.name} on ${id}"><option value="">Keep open</option><option value="recover"${f.verdicts[id+'|'+k.name]==='recover'?' selected':''}>Recovered</option><option value="strike"${f.verdicts[id+'|'+k.name]==='strike'?' selected':''}>Entered in error</option></select></div>`).join('')+`<p class="quiet-note">Recovery keeps the case in history. Entered in error marks it as a correction.</p>`,footer(back(),button('save-resolve','Save outcomes','button primary','',!Object.values(f.verdicts).some(Boolean))));}
 if(c.view==='triage')return sheet(c,'Care instructions',scope(c),`${fieldSelect('Care instruction','triage',careOptions,f.triage)}<p class="quiet-note">Care determines which conditions need attention. ${f.subjects.filter(id=>!cases(pig(c,id)).length).length} selected pigs have no open case and will stay unchanged.</p>`,footer(back(),button('save-triage','Save instructions','button primary')));
...
function productionOverview(c,p){
 ...
 const summary='<section class="batch-averages"><div class="detail-heading"><h4>Averages</h4><small>'+avg.count+' completed '+(avg.count===1?'batch':'batches')+'</small></div>'+(avg.count?'<div class="production-highlights">'+metrics.map(([label,value,unit])=>'<div><strong>'+recordValue(value,unit)+'</strong><span>'+label+'</span></div>').join(''):'<p class="detail-empty facts-surface">No completed batch records.</p>')+'</section>';
 return summary+'<section class="detail-section production-batch-section"><div class="detail-heading"><h4>Batches</h4></div><div class="production-batch-list">'+(records.length?records.map(r=>{
  ...
  return button('pig-production-batch','<span class="production-batch-copy"><span><strong>Batch '+esc(r.batchId)+'</strong>'+(current?'<small class="batch-record-status">Current</small>':'')+'</span><span class="production-batch-result">'+esc(result)+'</span>'+(current?'':'<small>'+(litter?'Weaned':'Finished')+' · '+esc(r.finishedDate||'Date unknown')+(r.rating?' · '+esc(r.rating):'')+'</small>')+'</span>'+icon('chevron'),'production-batch-card facts-surface',p.id+'|'+r.id);
 }).join(''):'<p class="detail-empty facts-surface">No batch records.</p>')+'</div></section>';
}
...
function pigFeedingToday(p,pe){
 const amount=pe.mode==='adlib'?'Ad-lib':quantity(pigAllocation(p,pe))+' <small>kg/day</small>';
 return '<section class="facts-surface st-panel pig-plan-summary"><h4>'+esc(pe.formula||'Feed plan')+'</h4><div class="feeding-main-amount"><span>Current feed</span><strong>'+amount+'</strong></div></section>';
}
function pigFeedPage(c){
 const p=pig(c,c.pigId),pe=penOf(c,p.id);
 const body=pigFeedingToday(p,pe)+(pe.mode==='adlib'?'<p class="quiet-note">Feed is freely available. No daily curve applies.</p>':'<section class="detail-section pig-plan-curve"><div class="detail-heading"><h4>Base curve</h4></div>'+feedCurve(p)+'</section>');
 return sheet(c,'Feed plan',p.id,body,footer(back(),''));
}
function feedBody(c,pe){
 ...
 return '<section class="feeding-today">'+feedingHeader(pe)+'<div class="feeding-main-amount"><span>Daily feed</span><strong>'+(pe.mode==='adlib'?'Ad-lib':quantity(allocation(pe))+' <small>kg/day</small>')+'</strong></div></section>'+plans;
}
...
function recordFlowOverlay(c){
 ...
 if(c.view==='finding-edit')return sheet(c,'Edit finding',p.id+' · '+k.name,
  (k.feedLinked?'<p class="quiet-note">'+esc(k.name)+' · Body condition. Use Review feed adjustment to change the condition band.</p>':fieldSelect('Finding','findingLabel',[[k.name,k.name],...catalog.filter(x=>x.name!==k.name&&!cases(p).some(y=>y.name===x.name)).map(x=>[x.name,x.name])],f.findingLabel))+
  healthStateFields(f)+'<label class="field">Note · optional<textarea data-v2="caseNote">'+esc(f.caseNote||'')+'</textarea></label>',footer(back(),button('save-finding','Save changes','button primary')));
 return sheet(c,'Resolve or remove',p.id+' · '+k.name,fieldSelect('Outcome','outcome',[['recover','Recovered'],['strike','Entered in error']],f.outcome)+(k.feedLinked?'<p class="quiet-note">This also ends the linked feed adjustment and returns this pig to the base plan.</p>':'')+'<label class="field">Note · optional<textarea data-v2="closeNote">'+esc(f.closeNote||'')+'</textarea></label><p class="quiet-note">The finding leaves the active list. Its record and this change remain in the pig’s log.</p>',footer(back(),button('save-finding-close','Save outcome','button primary')));
}
...
function feedEditor(c){
 ...
 const controls='<div class="plan-meta feed-main-fields"><label class="field"><span>Feeding mode</span><select data-feed-edit="feedingState"><option value="adjust"'+(!held&&!stopped?' selected':'')+'>Follow curve</option><option value="hold"'+(held?' selected':'')+'>Hold amount</option><option value="stop"'+(stopped?' selected':'')+'>Stop feed</option></select></label><div class="field"><div class="feed-adjustment-heading"><span>Adjustment · %</span>'+button('feed-edit-reset','Reset','text-button feed-reset').replace('<button','<button aria-label="Reset to curve"')+'</div><input type="number" min="-50" max="50" step="any" inputmode="decimal" data-feed-edit="percent" aria-label="Curve adjustment percent" placeholder="Mixed" value="'+(stopped||mixed?'':pct)+'"'+disabled+'></div></div>'+ 
...
function inspectionSurface(c){
 ...
 if(AstraSurfaces.isPage('inspection',c.view,c)||c.isDrawerBackground||['record-optional','feed-date','feed-note','medicine-picker','bulk-health-picker'].includes(c.view))return content;
...
function installDetailNavigation(){
 ...
 gallery.addEventListener('click',e=>{
  ...
  if(['bulk-pick-health','medicine-open','record-optional-open','feed-optional-open'].includes(a)){c.form.parentListScroll=root.querySelector('.bulk-pig-list,.feed-pig-rows')?.scrollTop||0;}
...
gallery.addEventListener('change',e=>{if(!e.target.matches('[data-transfer]'))return;e.stopImmediatePropagation();const root=e.target.closest('[data-card]'),i=+root.dataset.card,c=states[i];c.transferDraft[e.target.dataset.transfer]=e.target.value;if(e.target.dataset.transfer==='unit')c.transferDraft.pen='';render(i);},true);
